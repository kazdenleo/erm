...
export const ProductForm = React.forwardRef(function ProductForm({
  ...
  const buildProductSubmitPayload = () => {
    ...
    const payload = {
      ...
      mp_ozon_name: (() => {
        const attr = findOzonNameAttrs(ozonAttributes)[0];
        if (!attr) return trimOrNull(formData.mp_ozon_name);
        return trimOrNull(ozonAttrPlainText(ozonAttributeValues[String(attr.id)]));
      })(),
      mp_ozon_description: (() => {
        const attr = findOzonAnnotationAttrs(ozonAttributes)[0];
        const key = attr ? String(attr.id) : String(OZON_ANNOTATION_ATTR_ID);
        if (!attr && ozonAttributeValues[key] == null) {
          return trimOrNull(formData.mp_ozon_description);
        }
        return trimOrNull(ozonAttrPlainText(ozonAttributeValues[key]));
      })(),
            ...
            <div className="card-body">
              <div style={{ fontSize: '12px', color: 'var(--muted)', marginBottom: 10 }}>
                ...
                {ymCategoryAttributes.length > ymFormAttributes.length
                  ? `, без дублей упаковки/страны/названия: −${ymCategoryAttributes.length - ymFormAttributes.length}`
                  : ''}
                ), включая пустые. Обязательные сверху. Артикул продавца — в блоке связи выше, артикул производителя — в блоке названия. Габариты упаковки и страна — выше; длина, ширина и высота товара — здесь.
              </div>
              {formData.categoryId && categoryDetailsLoading ? (
                <div className="text-muted" style={{ fontSize: '12px' }}>Загрузка данных категории…</div>
              ) : !formData.categoryId ? (
