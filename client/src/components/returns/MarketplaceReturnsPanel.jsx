/**
 * Список возвратов с маркетплейсов (Ozon, WB, Яндекс).
 */
...
export function MarketplaceReturnsPanel({ embedded = false, onAcceptReturn }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (errs && typeof errs === 'object') {
        ...
        setWarnings(Object.entries(errs).map(([k, msg]) => `${labels[k] || k}: ${msg}`));
      }
      bumpReturnsStats();
    } catch (e) {
  ...
  return (
    <section className={`mp-returns-page${embedded ? ' mp-returns-page--embedded' : ''}`}>
      {!embedded ? (
          ...
          <table className="table mp-returns-table">
            ...
            <tbody>
              {items.map((row) => {
                ...
                const canAccept = showAcceptColumn && row.waitingPickup;
                return (
                  <tr
                  ...
                  >
                    ...
                    <td>
                      ...
                      {row.statusDetail ? <div className="mp-returns-sub">{row.statusDetail}</div> : null}
                    </td>
                    <td>
                      <div>{row.sku ?? '—'}</div>
                    ...
                    <td>
                      ...
                      {row.productExtra ? <div className="mp-returns-sub">{row.productExtra}</div> : null}
                    </td>
                    <td>
                      <div>{row.pickupAddress || '—'}</div>
                      {row.pickupPointId ? <div className="mp-returns-sub">ID: {row.pickupPointId}</div> : null}
                    </td>
                    ...
                    {showAcceptColumn ? (
                      <td className="mp-returns-actions-cell">
                        {canAccept ? (
                          <Button type="button" size="small" onClick={() => onAcceptReturn(row)}>
                            Принять возврат
                          </Button>
                        ) : (
                          <span className="mp-returns-sub">—</span>
                        )}
                      </td>
                    ) : null}
                  </tr>
