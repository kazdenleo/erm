  ...
  return (
    <div className="sales-analytics fbo-sales-analytics">
      <PageTitle
        ...
        title="Продажи FBO"
        subtitle="По уже загруженным отчётам (ночью и вручную). Смена фильтров только пересчитывает; свежие данные с МП — кнопкой «Загрузить с маркетплейсов»"
      />
      ...
      <p className="sales-analytics__hint">
        Таблица и карточки всегда строятся по отчётам, уже сохранённым в ERP (ночная загрузка и ручная
        кнопка). Смена периода или маркетплейса только пересчитывает эти данные — без запросов к API МП.
        Чтобы подтянуть свежий период вне расписания, нажмите «Загрузить с маркетплейсов» (WB —
        reportDetailByPeriod FBW/FBO, Ozon — finance/transaction/list FBO, Яндекс — united-netting FBY). Во
        вкладке «По заказам»: цена продажи, затраты (удержания МП, без себестоимости и доп. расходов) и сколько
        пришло от маркетплейса. Одна строка на продажу WB (затраты из других операций отчёта подтягиваются по
        заказу/штрихкоду). В сводке число заказов = только операции «Продажа», не служебные строки отчёта.
