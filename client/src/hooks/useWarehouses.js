...
import { invalidateWarehouseListCache } from '../services/warehouseListCache.js';
...
export function useWarehouses() {
  ...
  const loadWarehouses = useCallback(async (organizationId, options = {}) => {
    ...
    const keepVisible = options.silent === true;
    try {
  ...
  const updateWarehouse = async (id, updates) => {
    try {
      ...
      clearWarehousesCache();
      setWarehouses((prev) =>
        prev.map((w) => (String(w.id) === String(id) ? { ...w, ...updated } : w))
      );
      return updated;
    } catch (err) {
  ...
  const deleteWarehouse = async (id) => {
    try {
      ...
      invalidateWarehouseListCache();
      clearWarehousesCache();
      setWarehouses((prev) => prev.filter((w) => String(w.id) !== String(id)));
    } catch (err) {
