...
export function warehouseMappingMarketplaceHint(marketplace) {
  ...
  if (mp === 'ym') {
    return (
      'Для Яндекс.Маркет нужны два разных ID из кабинета: ' +
      'campaignId (ID кампании/магазина из API) — для заказов и резерва, ' +
      'и ID склада — для выгрузки остатков. Это не одно и то же число.'
    );
  }
  return (
