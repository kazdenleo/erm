...
export async function findOpenAutoPurchaseId(
  client,
  { profileId, supplierId, arrivalBucket, now = new Date() }
) {
  ...
  if (exactRow) {
    ...
    if (!parsed || parsed === bucket) {
      if (
        !isPurchaseCreatedInProcurementWindow(exactRow.created_at, bucket, now)
      ) {
        return null;
      }
...
export async function upgradeLegacyPurchaseArrivalNote(
  ...
  if (!isPurchaseCreatedInProcurementWindow(row?.created_at, bucket, now)) return;
