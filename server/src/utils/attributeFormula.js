...
/**
 * @returns {Array<{ kind: 'field'|'attr', key?: string, attrId?: string, name?: string, raw: string }>}
 */
...
/**
 * @returns {{ ok: true, value: number } | { ok: false, error: string, missing?: string[] }}
 */
...
function topoSortComputed(computedAttrs, attributes) {
  ...
  for (const [, set] of deps) {
    for (const d of set) incoming.set(d, (incoming.get(d) || 0) + 0);
  }
...
/**
 * Пересчитывает вычисляемые атрибуты. Ручные значения не трогает.
