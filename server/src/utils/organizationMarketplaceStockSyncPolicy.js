/**
 * Политика передачи остатков склада на маркетплейсы: организация и пользовательские категории.
 */
 ...
 * @returns {Promise<boolean>} true — не отправлять остатки на МП
 */
export async function isMarketplaceStockPushDisabled(organizationId) {
 ...
 * @returns {Promise<{ allowed: boolean, organizationId?: number, blockedBy?: 'organization'|'user_category', userCategoryId?: number, userCategoryName?: string }>}
 */
export async function assertMarketplaceStockPushAllowed(ctx = {}) {
  const organizationId = ctx.organizationId ?? null;
  if (await isMarketplaceStockPushDisabled(organizationId)) {
    logger.info('[MP Stock Push] skipped by organization setting skip_marketplace_stock_sync', {
      organizationId,
