/**
 * Видимость разделов меню для пользователя.
 * В БД nav_sections: { "products": false } — скрыть; по умолчанию всё видно.
 */
...
export function resolveNavSectionsForUser(user, profile) {
  if (!user) return {};
  if (user.role === 'admin') return {};
  if (user.is_profile_admin === true || user.isProfileAdmin === true) return {};
  const ar = normalizeAccountRoleKey(user.account_role ?? user.accountRole);
  if (ar === 'admin') return {};
  const roleNav = profile?.role_nav_sections ?? profile?.roleNavSections ?? {};
  return resolveNavSectionsForAccountRole(roleNav, ar || 'editor');
}
...
export function navSectionKeyForPath(pathname, search = '') {
  ...
  if (path.startsWith('/returns') || path.startsWith('/wb-returns')) return 'warehouse_return_customer';
