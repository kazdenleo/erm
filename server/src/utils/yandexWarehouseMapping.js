 ...
 * - legacy: "149210464" → campaignId (как в заказах)
 * - новый: "campaignId=149210464;warehouseId=2384892"
 */
...
export function parseYandexWarehouseMapping(raw) {
  ...
  // Старый формат: одно число = campaignId (ключ заказов), не warehouseId из кабинета.
  if (/^\d+$/.test(value)) {
