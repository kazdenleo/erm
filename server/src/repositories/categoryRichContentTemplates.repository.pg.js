/**
 * Шаблоны Rich-контента по пользовательским категориям и общий шаблон профиля
 */

import { query } from '../config/database.js';
import { normalizeRichContentModules } from '../utils/richContentTemplate.js';
...
function rowToTemplate(row) {
  ...
  return {
    ...
    modules: normalizeRichContentModules(row.modules),
...
export const categoryRichContentTemplatesRepository = {
  async findAllByProfile(profileId) {
    ...
    const result = await query(
      `SELECT t.*, uc.name AS category_name
       FROM category_rich_content_templates t
  ...
  async upsert({ userCategoryId, profileId, modules }) {
    const result = await query(
      `INSERT INTO category_rich_content_templates (
         user_category_id, profile_id, modules, updated_at
       ...
       RETURNING *`,
      [userCategoryId, profileId, JSON.stringify(normalizeRichContentModules(modules))]
    );
    const row = rowToTemplate(result.rows[0]);
    return row ? { ...row, source: 'category' } : null;
  },

  async upsertShared({ profileId, modules }) {
    const result = await query(
      `INSERT INTO category_rich_content_templates (
         user_category_id, profile_id, modules, updated_at
       ...
       ON CONFLICT (profile_id) WHERE user_category_id IS NULL DO UPDATE SET
         ...
         updated_at = CURRENT_TIMESTAMP
       RETURNING *`,
      [profileId, JSON.stringify(normalizeRichContentModules(modules))]
    );
    const row = rowToTemplate(result.rows[0]);
    return row ? { ...row, source: 'shared' } : null;
  },
  ...
  async deleteByCategoryId(userCategoryId, profileId = null) {
    ...
    const result = await query(
      `DELETE FROM category_rich_content_templates
  ...
  async deleteSharedByProfile(profileId) {
  ...
  async deleteAllCategoryTemplates(profileId) {
    if (profileId == null) return 0;
    const result = await query(
      `DELETE FROM category_rich_content_templates
       WHERE profile_id = $1::bigint AND user_category_id IS NOT NULL
       RETURNING id`,
      [profileId]
    );
