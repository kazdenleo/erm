/**
 * Шаблоны быстрых ответов на отзывы маркетплейсов
 */
...
class ReviewReplyTemplatesRepositoryPG {
    ...
    if (order == null || !Number.isFinite(Number(order))) {
      const maxRes = await query(
        `SELECT COALESCE(MAX(sort_order), -1) + 1 AS next_order
         FROM review_reply_templates WHERE profile_id = $1`,
        [pid]
      );
      order = Number(maxRes.rows[0]?.next_order ?? 0);
    }
    const result = await query(
      `INSERT INTO review_reply_templates (profile_id, title, body, sort_order, updated_at)
  ...
  async delete(id, profileId) {
    ...
    const result = await query(
      'DELETE FROM review_reply_templates WHERE id = $1 AND profile_id = $2',
      [nid, pid]
    );
...
export default new ReviewReplyTemplatesRepositoryPG();
