...
echo "==> health (Node :$PORT)"
curl -sf "http://127.0.0.1:$PORT/health" && echo "" || echo "health FAILED on :$PORT"
echo "==> health via nginx /api/health"
curl -sf -o /dev/null -w "HTTP %{http_code}\n" "http://127.0.0.1/api/health" 2>/dev/null || echo "nginx /api/health not reachable"
echo "==> api proxy smoke (expect 401 without token)"
curl -sf -o /dev/null -w "HTTP %{http_code}\n" "http://127.0.0.1/api/auth/me" 2>/dev/null || echo "nginx /api not reachable"
echo ""
